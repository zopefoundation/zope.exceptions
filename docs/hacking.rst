...
The main repository for :mod:`zope.exceptions` is in the Zope Subversion
repository:
...
.. code-block:: sh
   ...
   $ svn checkout svn://svn.zope.org/repos/main/zope.exceptions/trunk zope.exceptions
...
- The ``py26`` environment builds a ``virtualenv`` with ``python2.6``,
  installs :mod:`zope.exceptions`, and runs the tests
...
:mod:`zope.exceptions` tracks its bugs on Launchpad:

https://bugs.launchpad.net/zope.exceptions
...
.. code-block:: sh
   ...
   $ svn diff > zope.exceptions-cool_feature.patch
